...
const part1 = (rawInput) => {
  ...
  // Array of Array of Positions of Types
  const lists = rawInput.split(/[\n]*[a-z- ]*:[ ]*/g).filter((l) => l).map((l) => l.trim().split(/[\n]+/g).map((n) => n.split(/[\n ]/).map((m) => parseInt(m, 10))));

  const min = lists[0][0].reduce((min, seedIndex) => {
    let currIndex = seedIndex
    for (let i = 1; i < lists.length; i++) {
      const map = lists[i].find((m) => currIndex >= m[1] && currIndex < m[1] + m[2]);
      map && (currIndex = currIndex - map[1] + map[0]);
    }
    return Math.min(min, currIndex);
  }, Infinity);
